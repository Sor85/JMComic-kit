...
export function initFormEvents() {
  document
    .getElementById("btnCreateAutoTask")
    ?.addEventListener("click", showCreateForm);
  document
    .getElementById("btnCloseAutoForm")
    ?.addEventListener("click", hideForm);
  document
    .getElementById("btnCancelAutoForm")
    ?.addEventListener("click", hideForm);
  document.getElementById("autoForm")?.addEventListener("submit", handleSubmit);
  document
    .getElementById("toggleAutoPwd")
    ?.addEventListener("click", togglePasswordVisibility);

  // 压缩配置显示/隐藏
  const autoEnableCompression = document.getElementById(
    "autoEnableCompression",
  );
  const autoCompressionConfig = document.getElementById(
    "autoCompressionConfig",
  );
  if (autoEnableCompression && autoCompressionConfig) {
    autoEnableCompression.addEventListener("change", () => {
      autoCompressionConfig.style.display = autoEnableCompression.checked
        ? "block"
        : "none";
    });
  }
...
async function handleSubmit(e) {
  ...
  const form = e.target;
  const formContainer =
    form.closest(".forward-form-card") || form.closest("#autoFormCard");

  const getElement = (id) => formContainer.querySelector(`#${id}`);

  const editingId = form.dataset.editingId;
  const batchAlbumsCount = parseInt(
    getElement("autoBatchAlbumsCount").value,
    10,
  );
  const batchIntervalMinutes = parseInt(
    getElement("autoBatchIntervalMinutes").value,
    10,
  );
  ...
  const data = {
    ...
    dir_rule:
      getElement("autoDirRule").value.trim() || "Aauthoroname/Pindextitle",
    batch_albums_count:
      batchAlbumsCount > 0 && batchAlbumsCount <= 500 ? batchAlbumsCount : 50,
    batch_interval_minutes:
      batchIntervalMinutes > 0 && batchIntervalMinutes <= 1440
...
export async function editTask(id) {
  try {
    ...
    const taskCard = document
      .querySelector(`[data-action="auto-edit"][data-task-id="${id}"]`)
      ?.closest(".auto-task-card");
    if (!taskCard) {
    ...
    // 4. 加载任务详情
    const task = await api.fetchTaskDetail(id);
    ...
    console.log(
      `[编辑表单] 找到 ${clonedWrappers.length} 个克隆的 custom-select-wrapper`,
    );
...
function bindClonedFormEvents(formClone) {
  ...
  selects.forEach((select, index) => {
    console.log(
      `[编辑表单] 正在初始化第 ${index + 1} 个 select: ${select.id || "(无ID)"}`,
    );
  ...
  if (enableCompression && compressionConfig) {
    enableCompression.addEventListener("change", () => {
      compressionConfig.style.display = enableCompression.checked
        ? "block"
        : "none";
    });
  }
...
function fillFormData(formClone, task, id) {
  ...
  formClone.querySelector("#autoDirRule").value =
    task.dir_rule || "Aauthoroname/Pindextitle";

  // 兼容旧数据
  formClone.querySelector("#autoBatchAlbumsCount").value =
    task.batch_albums_count || task.batch_size || 50;
  formClone.querySelector("#autoBatchIntervalMinutes").value =
    task.batch_interval_minutes || 30;

  // 回填压缩配置
  const compression = task.compression || {};
  const autoEnableCompression = formClone.querySelector(
    "#autoEnableCompression",
  );
  const autoCompressionConfig = formClone.querySelector(
    "#autoCompressionConfig",
  );
  if (compression.enabled) {
    autoEnableCompression.checked = true;
    autoCompressionConfig.style.display = "block";
    formClone.querySelector("#autoCompressionFormat").value =
      compression.format || "zip";
    formClone.querySelector("#autoCompressionLevel").value =
      compression.level || "album";
    formClone.querySelector("#autoCompressionPassword").value = "";
    formClone.querySelector("#autoDeleteAfterCompress").checked =
      compression.delete_original || false;
  } else {
  ...
  if (pdf.enabled) {
    ...
    formClone.querySelector("#autoDeleteAfterPdf").checked =
      pdf.delete_original || false;
  } else {
  ...
  try {
    ...
    if (result.error) {
      ...
      const card =
        document
          .querySelector(
            `.auto-task-card [data-action="auto-start"][data-task-id="${id}"]`,
          )
          ?.closest(".auto-task-card") ||
        document
          .querySelector(`.auto-task-card .icon-stop-btn[data-task-id="${id}"]`)
          ?.closest(".auto-task-card");
